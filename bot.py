...
CAVIAR_CANDIDATES = [
    os.path.join(USER_FONT_DIR, "CaviarDreams.ttf"),
    os.path.join(os.getcwd(), "CaviarDreams.ttf"),
    "/mnt/data/CaviarDreams.ttf",
]
...
INTRO_INLINE_CANDIDATES = [
    os.path.join(USER_FONT_DIR, "IntroInline.ttf"),
    os.path.join(os.getcwd(), "IntroInline.ttf"),
    "/mnt/data/IntroInline.ttf",
]

INTER_EXTRA_BOLD_CANDIDATES = [
    os.path.join(USER_FONT_DIR, "Inter-ExtraBold.ttf"),
    os.path.join(os.getcwd(), "Inter-ExtraBold.ttf"),
    "/mnt/data/Inter-ExtraBold.ttf",
]
...
FONT_CAVIAR = next((p for p in CAVIAR_CANDIDATES if os.path.exists(p)), None)
FONT_MONTSERRAT_BOLD = next((p for p in MONTSERRAT_BOLD_CANDIDATES if os.path.exists(p)), None)
FONT_MONTSERRAT_BLACK = next((p for p in MONTSERRAT_BLACK_CANDIDATES if os.path.exists(p)), None)
FONT_INTRO_INLINE = next((p for p in INTRO_INLINE_CANDIDATES if os.path.exists(p)), None)
FONT_INTER_EXTRA_BOLD = next((p for p in INTER_EXTRA_BOLD_CANDIDATES if os.path.exists(p)), None)
FONT_SYSTEM_REG = next((p for p in SYSTEM_REG_CANDIDATES if os.path.exists(p)), None)
FONT_SYSTEM_BOLD = next((p for p in SYSTEM_BOLD_CANDIDATES if os.path.exists(p)), None)

if FONT_SYSTEM_REG is None or FONT_SYSTEM_BOLD is None:
...
def load_font(
    ...
    *,
    ...
    intro_inline: bool = False,
    inter_extra_bold: bool = False,
    bold: bool = False,
) -> ImageFont.FreeTypeFont:
    if caviar and FONT_CAVIAR:
        return ImageFont.truetype(FONT_CAVIAR, size=size)
    if montserrat_black and FONT_MONTSERRAT_BLACK:
        return ImageFont.truetype(FONT_MONTSERRAT_BLACK, size=size)
    if montserrat_bold and FONT_MONTSERRAT_BOLD:
        return ImageFont.truetype(FONT_MONTSERRAT_BOLD, size=size)
    if intro_inline and FONT_INTRO_INLINE:
        return ImageFont.truetype(FONT_INTRO_INLINE, size=size)
    if inter_extra_bold and FONT_INTER_EXTRA_BOLD:
        return ImageFont.truetype(FONT_INTER_EXTRA_BOLD, size=size)
    path = FONT_SYSTEM_BOLD if bold else FONT_SYSTEM_REG
...
def main_menu() -> ReplyKeyboardMarkup:
    ...
    kb.row(KeyboardButton("📰 Собрать новости"), KeyboardButton("🖼 Оформить новость"))
    return kb
...
def make_card_am(photo_bytes: bytes, title: str) -> BytesIO:
    photo = open_image_from_bytes(photo_bytes).convert("RGB")
    img = fit_cover(photo, CARD_W, CARD_H)

    top_h = int(CARD_H * 0.20)
    blur_radius = 18
    blur_mix = 0.50

    top_crop = img.crop((0, 0, CARD_W, top_h))
    top_blur = top_crop.filter(ImageFilter.GaussianBlur(radius=blur_radius))
    top_mix = Image.blend(top_crop, top_blur, blur_mix)

    overlay = Image.new("RGBA", (CARD_W, top_h), (0, 0, 0, 95))
    top_mix_rgba = top_mix.convert("RGBA")
    top_final = Image.alpha_composite(top_mix_rgba, overlay).convert("RGB")
...
def send_preview_message(chat_id: int, item: dict):
    ...
    caption = (
        ...
        f"{html.escape(text)}\n\n"
        f"<a href='{html.escape(item['link'])}'>Источник</a>"
    )
...
# TEXT MENU ACTIONS
# =========================
@bot.message_handler(func=lambda m: m.text == "📰 Собрать новости")
...
@bot.message_handler(func=lambda m: m.text == "🖼 Оформить новость")
def menu_format(message):
    kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    kb.row(KeyboardButton("МН"), KeyboardButton("ЧП ВМ"))
    kb.row(KeyboardButton("АМ"), KeyboardButton("Сторис ФДР"))
...
def on_photo(message):
    ...
    bot.reply_to(message, "Фото получено, но сейчас я его не жду. Нажми «🖼 Оформить новость».")
...
def on_text(message):
    ...
    if flow == "choose_template":
        if text == "МН":
            ...
            set_state(message.from_user.id, template="am", flow="await_manual_photo")
            bot.send_message(message.chat.id, "Пришли фото:", reply_markup=telebot.types.ReplyKeyboardRemove())
            return
    ...
    if flow == "format_from_news":
        ...
        if text == "МН":
            ...
            if not photo_bytes:
                ...
                return
            title = article.get("title") or item["title"]
            card = make_card_am(photo_bytes, title)
            caption = build_publish_caption(title, article.get("text") or item.get("description") or "")
            post_id = sha1s(str(time.time()) + title)
            pending = st.get("pending_posts", {})
            pending[post_id] = {
            ...
            set_state(
                ...
                source_url=item["link"],
                news_title=article.get("title") or item["title"],
