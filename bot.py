...
# =========================
# Проверка доступности видео-библиотек
# =========================
try:
    import numpy as np
    from moviepy.editor import VideoFileClip
...
NEWS_BATCH_SIZE = 20
NEWS_TIMEOUT = 30  # Таймаут для каждого сайта
...
# =========================
NEWS_FIRST_BATCH = 20
NEWS_PER_SOURCE_CAP = 5  # Максимум 5 новостей с одного источника за раз
...
@retry_on_error()
def http_get(url: str, timeout: int = NEWS_TIMEOUT, headers: dict = None) -> Optional[str]:
    """Быстрый HTTP GET с таймаутом"""
    if not validate_url(url):
        return None
    try:
...
@retry_on_error()
def http_get_bytes(url: str, timeout: int = NEWS_TIMEOUT) -> Optional[bytes]:
    if not validate_url(url):
        return None
    try:
        ...
        logger.debug(f"HTTP bytes error for {url}: {e}")
        return None
...
def parse_rss(url: str, source_name: str, limit: int = NEWS_PER_SOURCE_CAP) -> List[Dict]:
    try:
        xml_text = http_get(url, timeout=NEWS_TIMEOUT)
        if not xml_text:
            return []
        root = ET.fromstring(xml_text)
    except Exception as e:
        logger.debug(f"Failed to parse RSS {url}: {e}")
        return []
...
def _candidate_links_from_page(start_url: str, page_html: str, domain: str,
                               ...
                               max_candidates: int = 20) -> List[Tuple[str, str]]:
    if not page_html:
        return []
...
def parse_html_og_source(source: Dict, limit: int = NEWS_PER_SOURCE_CAP) -> List[Dict]:
    ...
    for start_url in start_urls:
        try:
            timeout = source.get("timeout", NEWS_TIMEOUT)
            ...
            logger.debug(f"[NEWS] {source['name']} start error: {e}")
            continue
    ...
    for href, anchor in candidates:
        try:
            ...
            logger.debug(f"[NEWS] {source['name']} article error: {e}")
            continue
    return out
...
def fetch_article_full_text_generic(url: str) -> str:
    ...
    try:
        ...
        page_html = http_get(url, timeout=NEWS_TIMEOUT, headers=headers)
...
def news_sources_kb():
    ...
    for source_id, source_name in SOURCE_NAMES.items():
        buttons.append(InlineKeyboardButton(source_name, callback_data=f"news_src:{source_id}"))
    
    # Добавляем кнопки в клавиатуру (по 2 в ряд)
    kb.add(*buttons)
    
    # Добавляем кнопку для всех источников
    kb.row(
        InlineKeyboardButton("🌐 Все сайты", callback_data="news_src:all"),
        InlineKeyboardButton("❌ Отмена", callback_data="news_src:cancel")
    )
...
# =========================
def fetch_news_from_sources(source_ids: List[str], limit_per_source: int = NEWS_PER_SOURCE_CAP) -> List[Dict]:
    """Собирает новости только из указанных источников (быстро)"""
    merged: List[Dict] = []
    by_url = set()
    
    # Если выбран конкретный источник
    if len(source_ids) == 1 and source_ids[0] != "all":
        selected_sources = [src for src in NEWS_SOURCES if src["id"] == source_ids[0]]
    else:
        # Все источники
        selected_sources = NEWS_SOURCES
    
    start_time = time.time()
    max_total_time = 90  # Максимум 1.5 минуты на всё
    
    for src in selected_sources:
        # Проверяем общее время
        if time.time() - start_time > max_total_time:
            logger.info(f"Stopping news fetch due to time limit")
            break
            
        kind = src["kind"]
        try:
            if kind == "rss":
                items = parse_rss(src["url"], src["name"], limit=limit_per_source)
            elif kind == "html_og":
                items = parse_html_og_source(src, limit=limit_per_source)
            else:
                items = []
            ...
            logger.debug(f"[NEWS] {src['name']} error: {e}")
            continue
    ...
    # Возвращаем первые NEWS_FIRST_BATCH
    return merged[:NEWS_FIRST_BATCH]


def fetch_all_news_last24h() -> List[Dict]:
    """Собирает новости со всех источников"""
    return fetch_news_from_sources(["all"])
...
# =========================
# Улучшение качества изображения (только резкость +20% и насыщенность +15%)
# =========================
def enhance_image_quality(image_bytes: bytes) -> BytesIO:
    """
    Улучшает качество изображения:
    - Увеличивает резкость на 20%
    - Увеличивает насыщенность на 15%
    """
    try:
        # Открываем изображение
        img = Image.open(BytesIO(image_bytes)).convert("RGB")
        
        # Увеличиваем резкость на 20%
        enhancer_sharpness = ImageEnhance.Sharpness(img)
        img = enhancer_sharpness.enhance(1.20)
        
        # Увеличиваем насыщенность на 15%
...
class NewsAutoPublisher:
    ...
    def publish_news_digest(self, manual=False):
        try:
            ...
            self._send_news_batch(self.chat_id, 0, len(items), manual)
    ...
    def _send_news_batch(self, chat_id, start_idx, count, manual=False):
        ...
        for i in range(start_idx, end_idx):
            ...
            if image_url:
                try:
                    ...
                    logger.debug(f"Error sending photo: {e}")
                    bot.send_message(
                        chat_id, 
                        f"<b>{html.escape(title)}</b>\n\n📰 {html.escape(source)}",
                        parse_mode="HTML",
        ...
        if end_idx < len(items):
            ...
            more_kb.row(InlineKeyboardButton(f"📥 Загрузить еще {min(10, len(items)-end_idx)}", callback_data=f"load_more:{chat_id}"))
...
def on_read_full_news(c):
    ...
    try:
        ...
        bot.send_message(c.message.chat.id, "⏳ Загружаю полный текст...")
...
def on_load_more(c):
    ...
    news_publisher._send_news_batch(chat_id, current_idx, 10, manual=True)
    bot.answer_callback_query(c.id, f"Загружаю еще 10 новостей...")
...
def cmd_news(message):
    ...
    if str(uid) == str(AUTO_NEWS_CHAT_ID):
        # Если это чат для авто-новостей, просто запускаем выгрузку
        bot.send_message(message.chat.id, "🔍 Собираю новости со всех источников...", reply_markup=main_menu_kb())
        items = fetch_all_news_last24h()
        if items:
            set_news_cache(uid, items)
            news_publisher._send_news_batch(message.chat.id, 0, len(items), manual=True)
        else:
            bot.send_message(message.chat.id, "😕 Новостей не найдено", reply_markup=main_menu_kb())
        return
    
    # Для обычных пользователей показываем выбор источника
    st = user_state.get(uid) or {}
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("news_src:"))
def on_news_source_select(c):
    ...
    if action == "all":
        ...
        source_name = SOURCE_NAMES.get(action, action)
        items = fetch_news_from_sources([action])
    ...
    if items:
        ...
        if cache:
            ...
            # Если есть еще новости, показываем кнопку "еще"
            if end_idx < len(items):
                more_kb = InlineKeyboardMarkup()
                more_kb.row(InlineKeyboardButton(f"📥 Загрузить еще {min(10, len(items)-end_idx)}", callback_data="nmore"))
                bot.send_message(c.message.chat.id, f"📊 Показано {end_idx} из {len(items)} новостей\nОсталось: {len(items)-end_idx}", reply_markup=more_kb)
            else:
                bot.send_message(c.message.chat.id, "✅ Все новости загружены!", reply_markup=main_menu_kb())
    else:
        bot.send_message(c.message.chat.id, f"😕 Новостей из {source_name} не найдено", reply_markup=main_menu_kb())
    
    st.pop("news_step", None)
    user_state[uid] = st
    bot.answer_callback_query(c.id, f"✅ Загружено {len(items)} новостей")
...
def on_photo(message):
    ...
    # Остальная обработка фото для постов
    ...
    # Стандартная обработка для других шаблонов
    try:
        file_id = message.photo[-1].file_id
        photo_bytes = tg_file_bytes(file_id)
...
def on_document(message):
    ...
    if doc.mime_type and doc.mime_type in video_mime_types:
        try:
            ...
            if step in ["waiting_video_for_gif", "waiting_video_for_edit", "waiting_video_template"]:
                ...
                # Вызываем обработчик видео
                from types import SimpleNamespace
                video_msg = SimpleNamespace(
                    chat=message.chat,
                    from_user=message.from_user,
                    message_id=message.message_id,
                    video=SimpleNamespace(file_id=doc.file_id, file_size=doc.file_size)
                )
                on_video(video_msg)
                return
            else:
    ...
    if doc.mime_type and doc.mime_type.startswith("image/"):
        try:
            ...
            # Для остальных случаев обрабатываем как обычное фото
            warn_if_too_small(message.chat.id, photo_bytes)
            st["photo_bytes"] = photo_bytes
    ...
    # Если не видео и не изображение
...
def on_text(message):
    ...
    if step == "waiting_title_fdr_post":
        ...
        bot.reply_to(message, f"✅ Заголовок сохранён!\n\n<b>{html.escape(text)}</b>\n\n🎯 Теперь отправь <b>ФРАЗУ</b>, которую нужно выделить фиолетовой плашкой:", parse_mode="HTML")
        return
